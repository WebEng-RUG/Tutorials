...
This repository currently contains an API specification (in `./specification`) in OpenAPI 3.x format, as well as two different backend implementations, one using ExpressJS with TypeScript and one using the ASP.NET Core framework. Additionally, a front-end was built using vanilla JS without any third-party libraries.
