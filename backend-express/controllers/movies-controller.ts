...
export class MoviesController {
    public async getAllAsync (req: Request, res: Response): Promise<void> {
        // In Express, we need to convert our request query to actuall class instances ourselves.
        ...
        let query = getConnection().getRepository(Movie).createQueryBuilder("movie");
    ...
    public async createAsync (req: Request, res: Response): Promise<void> {
        ...
        movie.actors = await getConnection().getRepository(Actor)
            .createQueryBuilder("actor")
            .where("actor.name IN (:...names)", { names: apiMovie.actors.map(a => a.name) })
            .getMany();

        movie.languages = await getConnection().getRepository(Language)
    ...
    public async getMovieAsync (req: Request<{ id: number }>, res: Response): Promise<void> {
        let movie = await getConnection().getRepository(Movie).findOne(req.params.id);
    ...
    public async updateMovieAsync (req: Request<{ id: number }, {}, ApiMovie>, res: Response): Promise<void> {
        ...
        let movie = await movieRepository.findOne(req.params.id);
        if(!movie) {
            res.status(404);
            res.json();
            return;
